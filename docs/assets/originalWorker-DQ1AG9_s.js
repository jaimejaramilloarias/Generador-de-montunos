(function(){"use strict";var c=`"""Backend package exposing the montuno generation core."""
...
`)),B})());const A=await D;return await R(A),i||(i=A.globals.get("web_generate")),A}async function R(g){for(const A of U){if(s.has(A))continue;const B=_(A);u(g,`backend/reference_midi_loops/${A}`,B),s.add(A)}}async function x(g){const{id:A,payload:B}=g;try{const{baseUrl:n,...Q}=B,e=await f(String(n??"/"));if(!i)throw new Error("Python bridge no inicializado");const o=await i(JSON.stringify(Q)),V=typeof o=="string"?o:o.toString();typeof o.destroy=="function"&&o.destroy();const X=JSON.parse(V);E.postMessage({id:A,success:!0,result:X})}catch(n){const Q=n instanceof Error?n.message:"No se pudo generar el montuno.";E.postMessage({id:A,success:!1,error:Q})}}E.onmessage=g=>{const A=g.data;A.type==="generate"&&x(A)}})();
